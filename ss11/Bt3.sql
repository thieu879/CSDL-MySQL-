...
delimiter //
	create procedure GetCustomerByPhone(in PhoneNumber VARCHAR(15))
    begin
		select 
			CustomerID,
            FullName,
...
delimiter //
	create procedure GetTotalBalance(in CustomerID_ int, out TotalBalance int)
    begin
		select 
            sum(Balance) into TotalBalance
		from Accounts
...
delimiter //
	create procedure IncreaseEmployeeSalary(out Salary_Employees decimal(10,2), in Employee_ID int)
    begin
		update Employees
...
call IncreaseEmployeeSalary(@Salary_Employees_value, 4);
select @Salary_Employees_value as IncreaseEmployeeSalary;
